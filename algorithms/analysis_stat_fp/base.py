...
class Model(nn.Module):
    ...
    def fit(self, x, y):
        ...
        """
        # 还原时间序列
        seq = x.reshape(-1, x.shape[-1]).cpu().numpy()
        self.model = VAR(seq)
        self.model = self.model.fit(maxlags=self.lag)
        # 计算残差标准差
        residual = self.model.resid

        self.sigma = residual.std(axis=0)

    def forward(self, x):
        x_np = x.cpu().numpy()
        outputs = []
        for seq in x_np:
            pred = self.model.forecast(seq, steps=1)[0]
            # 计算异常概率
            error = np.abs(pred - seq[-1])
            score = np.mean(error / (self.sigma + 1e-6))
            fault_prob = 1 - np.exp(-score)
            pred = np.append(pred, fault_prob)
            outputs.append(pred)
    ...
    def state_dict(self, *args, **kwargs):
        ...
        if self.model is not None:
            ...
            state["model"] = buf.getvalue()
        if self.sigma is not None:
            state["sigma"] = self.sigma
        return state

    def load_state_dict(self, state_dict, strict=True):
        if "sigma" in state_dict:
...
def train_or_eval(
    ...
    if train:
        data_loader = DataLoader(data, len(data), shuffle=False)
