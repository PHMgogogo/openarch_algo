...
class Model(nn.Module):
    def __init__(self, n_estimators: int = 100, **kwargs):
        super().__init__()
        self.rf = RandomForestRegressor(n_estimators=n_estimators, **kwargs)

    def fit(self, x: torch.Tensor, y: torch.Tensor):
        self.rf.fit(x.cpu().numpy(), y.cpu().numpy().ravel())

    def forward(self, x: torch.Tensor):
        pred = self.rf.predict(x.cpu().numpy())
        pred = torch.as_tensor(pred, dtype=torch.float32).unsqueeze(-1)
        return pred
