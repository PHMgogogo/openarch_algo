...
class Model(nn.Module):
    ...
    def _make_output_proj(self, D: int, device, dtype) -> None:
        """根据输入维度创建输出投影层。"""
        self.output_proj = nn.Linear(self.d_model, D, device=device, dtype=dtype)
    ...
    def _load_from_state_dict(
        ...
        if self.output_proj is None and output_weight_key in state_dict:
            ...
            self._make_output_proj(w.shape[0], w.device, w.dtype)
